...
def gpt_prompt(user_input):
    ...
    [환자의 정보]는 {st.session_state.gpt_input_care}이며 이 정보를 바탕으로 예시를 참고하여 <규칙>에 따라 처방한다.
...
def gemini_prompt(user_input):
    ...
    환자의 성별은 {st.session_state.selected_gender}이며, 나이는 {st.session_state.selected_age}이다.
    [환자의 정보]는 {st.session_state.gemini_input_care}이며 이 정보를 바탕으로 예시를 참고하여 <규칙>에 따라 처방한다.
...
if "messages" not in st.session_state:
    st.session_state.messages = {
        "gpt": [
            {"role": "system", "content": "GPT가 사용자의 현재 정보로 심정지 발생 가능성을 알려주고, 예방하는 방법을 알려드립니다."}
        ],
        "gemini": [
            {"role": "model", "parts": [{"text": "Gemini가 사용자의 현재 정보로 심정지 발생 가능성을 알려주고, 예방하는 방법을 알려드립니다."}]}
        ]
    }
...
selected_chatbot = st.selectbox(
    "원하는 챗봇을 선택하세요.",
    options=["GPT를 통한 심정지위험 예방", "Gemini를 통한 심정지위험 예방"],
    placeholder="챗봇을 선택하세요.",
    help="선택한 LLM 모델에 따라 다른 챗봇을 제공합니다."
)

if selected_chatbot == "GPT를 통한 심정지위험 예방":
    colored_header(
...
elif selected_chatbot == "Gemini를 통한 심정지위험 예방":
    colored_header(
    ...
    st.button("대화 초기화", on_click=lambda: st.session_state.update({
        "messages": {"gemini": [{"role": "model", "parts": [{"text": "Gemini가 사용자의 현재 정보로 심정지 발생 가능성을 알려주고, 예방하는 방법을 알려드립니다."}]}]}
    }))
